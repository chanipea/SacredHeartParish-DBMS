...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $conn = new mysqli("localhost", "root", "", "parish");
    ...
    // Optional: You could add a check here to ensure the priestID exists in your `priest` table,
    // especially if users could somehow manipulate the POST data.
    // Example:
    // if (!empty($priestID) && ctype_digit($priestID)) {
    ...
    if (isset($_POST['submitRecord'])) {
        $sql = "INSERT INTO eventt (EventType, Date, OfficiatingPriestID, ParishStaffID)
                VALUES (?, ?, ?, ?)";
...
<head>
    <meta charset="UTF-8">
    <title>Events & Sacraments Management</title>
    <link rel="icon" href="/imagess/sacred.png">
    <link href='https://fonts.googleapis.com/css?family=Poppins' rel='stylesheet'>
    <link rel="stylesheet" href="eventstyle.css?v=8">
...
";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr data-priest-id='" . ($row["OfficiatingPriestID"] ?? '') . "'>"; // Add this line
                        echo "<td>" . ($row["EventID"] ?? '-') . "</td>";
                        echo "<td>" . ($row["EventType"] ?? '-') . "</td>";
                        echo "<td>" . ($row["Date"] ?? '-') . "</td>";
                        echo "<td>" . ($row["PriestName"] ?? '-') . "</td>";
                        echo "<td>" . ($row["CreatedBy"] ?? '-') . "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='5'>No event records found.</td></tr>";
                }
                $conn->close();
                ?>
                </tbody>
    ...
    <div class="modal" id="updateModal">
        <form class="modal-content" id="updateEventForm" method="POST" action="eventTEST.php" style="width: 1000px; height: 600px; background: #f3f3f3; border-radius: 8px; padding: 10px; position: relative;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-top: 90px; justify-content: space-between;">
                ...
                <div style="flex: 1 1 45%;">
                    ...
                    <select name="OfficiatingPriestID" id="updatePriestID" required style="width: 80%; padding: 5px; margin-left: 30px;">
                        ...
                        <?php
                        // Database connection
                        $connLocal = new mysqli("localhost", "root", "", "parish"); // Use a different var name if $conn is in wider scope
